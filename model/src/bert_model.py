...
class BertAttention(nn.Cell):
    ...
    def get_attention_probs(self, from_tensor, to_tensor, attention_mask):
        """reshape 2d/3d input tensors to 2d"""
        shape_from = F.shape(attention_mask)[2]
        from_tensor = F.depend(from_tensor, shape_from)
        from_tensor_2d = self.reshape(from_tensor, self.shape_from_2d)
...
class BertSelfAttention(nn.Cell):
    ...
    def get_attention_probs(self, input_tensor, attention_mask):
        attention_probs = self.attention.get_attention_probs(input_tensor, input_tensor, attention_mask)
        return attention_probs
...
class BertEncoderCell(nn.Cell):
    ...
    def get_attention_probs(self, hidden_states, attention_mask):
        attention_probs = self.attention.get_attention_probs(hidden_states, attention_mask)
        return attention_probs
...
class BertTransformer(nn.Cell):
    ...
    def get_attention_probs(self, input_tensor, attention_mask):
        ...
        for layer_module in self.layers:
            layer_output = layer_module(prev_output, attention_mask)
            layer_attention = layer_module.get_attention_probs(prev_output, attention_mask)
            print(layer_attention.shape)
            all_encoder_attention_probs = all_encoder_attention_probs + (layer_attention,)
            prev_output = layer_output
        ...
        return all_encoder_attention_probs
...
class BertModel(nn.Cell):
    ...
    def get_attention(self, input_ids, token_type_ids, input_mask):
        """Bidirectional Encoder Representations from Transformers."""
        # embedding
        embedding_tables = self.bert_embedding_lookup.embedding_table
        word_embeddings = self.bert_embedding_lookup(P.Cast()(input_ids, mindspore.int32))
        ...
        # bert encoder
        encoder_output = self.bert_encoder.get_attention_probs(self.cast_compute_type(embedding_output),
                                           attention_mask)
